...
def get_sub_systems() -> list[Subsystem]:
    data: list[dict[str, Any]] = parse_response_body(
        build_url("GetPodsystemy", secondId=1)
    )
...
def get_serving_places(subsystem_id: int) -> list[ServingPlace]:
    data: list[dict[str, Any]] = parse_response_body(
        build_url("GetVydejny", subsystem_id)
    )
...
def get_dish_types(subsystem_id: int) -> list[DishType]:
    data: list[dict[str, Any]] = parse_response_body(
        build_url("GetKategorie", subsystem_id)
    )
...
def get_dishes(subsystem_id: int) -> list[Dish]:
    data: list[dict[str, Any]] = parse_response_body(
        build_url("GetJidla", subsystem_id, 1)
    )
...
def get_info(subsystem_id: int) -> list[Info]:
    data: list[dict[str, Any]] = parse_response_body(
        build_url("GetInfo", subsystem_id, 1)
    )
    return [Info(x) for x in data]


def get_from_times(subsystem_id: int) -> list[OpenTime]:
    data: list[dict[str, Any]] = parse_response_body(
        build_url("GetOtDoby", subsystem_id, 1)
    )
...
def get_week_info(subsystem_id: int) -> list[WeekInfo]:
    data: list[dict[str, Any]] = parse_response_body(
        build_url("GetTydny", subsystem_id)
    )
...
def get_day_dish(week_id: int) -> list[DayDish]:
    data: list[dict[str, Any]] = parse_response_body(
        build_url("GetTydnyDny", secondId=week_id)
    )
