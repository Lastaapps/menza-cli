...
class ConfigLoader():
    ...
    def __parse_allergens(allergens : str) -> list[str]:
        if len(allergens) == 0:
            return list()
        return list(map( lambda x: str(int(x)), allergens.split(","),))
    ...
    def load_config(self) -> Result[AppConfig, str]:
        ...
        lasta_api_key = menza.get("lasta_api_key", "menza-cli_15becd42-cbae-48b2-aa69-650d06763454")
        lasta_api_url = menza.get("lasta_url", "https://lastaapps.sh.cvut.cz/menza")

        allergens = ConfigLoader.__parse_allergens(
            menza.get("allergens", "")
        )
        match allergens:
            case Ok(value):
        ...
        return Ok(AppConfig(
            agata_api_key, agata_url_base, agata_url_api,
            lasta_api_key, lasta_api_url,
            allergens,
        ))
